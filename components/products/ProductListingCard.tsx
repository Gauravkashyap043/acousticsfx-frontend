...
export function ProductListingCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex flex-1 flex-col ${isGrid ? "p-4 sm:p-5 lg:p-5 xl:p-6" : "p-5 sm:p-6 lg:p-7"}`}
      >
        ...
        <p
          className={`line-clamp-3 flex-1 leading-relaxed text-neutral-600 ${
            isGrid ? "text-[13px] sm:text-sm" : "text-sm sm:text-[15px]"
          }`}
        >
          {description}
        </p>
        ...
        <span
          className={`inline-flex items-center gap-2 font-semibold text-[#1F6775] transition-colors group-hover:text-[#EA8E39] ${isGrid ? "mt-4 text-xs sm:text-sm" : "mt-6 text-sm"}`}
        >
          View details
          <svg
