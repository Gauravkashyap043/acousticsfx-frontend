...
// Re-export extended SubProduct from API for detail pages (specs, gallery, etc.)
...
/** Other products in the category for the “Related products” strip (excludes current product). */
export async function fetchRelatedProductsForCategory(
  categorySlug: string,
  excludeProductSlug: string
): Promise<RelatedProductListItem[]> {
  const toItem = (p: Product): RelatedProductListItem => ({
    slug: p.slug,
    title: p.title,
...
export async function fetchMergedProduct(
  ...
  try {
    ...
    return {
      ...
      showTrademark: apiProduct.showTrademark ?? staticProduct?.showTrademark,
    };
  } catch {
...
/** Full sub-product for detail page (merged from API + static fallback) */
...
export async function fetchMergedSubProduct(
  ...
  try {
    ...
    const mergedSub: SubProductMerged = {
      ...
      showTrademark: apiResult.subProduct.showTrademark ?? staticSub?.showTrademark,
      specSectionTitle: apiResult.subProduct.specSectionTitle,
      ...
      certificationsSectionTitle: apiResult.subProduct.certificationsSectionTitle,
      certificationsSectionDescription: apiResult.subProduct.certificationsSectionDescription,
    ...
    const product = staticProduct
      ? {
          ...staticProduct,
          categorySlug: apiResult.product?.categorySlug ?? staticProduct.categorySlug,
          showTrademark: apiResult.product?.showTrademark ?? staticProduct.showTrademark,
        }
      : {
          ...
          showTrademark: apiResult.product.showTrademark,
        };
